    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                Log.i(TAG, "onInterceptHoverEvent: ACTION_DOWN");
                isIntercepted = false;
                break;
            case MotionEvent.ACTION_MOVE:
                Log.i(TAG, "onInterceptHoverEvent: ACTION_MOVE");
                int deltax = x - mLastX;
                int deltay = y - mLastY;
                if (Math.abs(deltay) > Math.abs(deltax)) {
                    Log.i(TAG, "onInterceptHoverEvent: 拦截");
                    isIntercepted = false;
                } else {
                    Log.i(TAG, "onInterceptHoverEvent: 不拦截");
                    isIntercepted = true;
                }
                break;
